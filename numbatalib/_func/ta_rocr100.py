...
@njit(cache=True)
def _rocr100_kernel(real: np.ndarray, timeperiod: int, out: np.ndarray) -> None:
    n = real.shape[0]
    if n <= timeperiod:
        return
    i = timeperiod
    while i < n:
        ...
        if old == 0.0:
            ...
            out[i] = real[i] / old * 100.0
        i += 1


def ROCR100(real, timeperiod: int = 10):
    """
    ...
    _rocr100_kernel(real_arr, tp, out)
    return out
