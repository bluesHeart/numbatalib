...
@njit(cache=True)
def _tsf_kernel(real: np.ndarray, timeperiod: int, out: np.ndarray) -> None:
    n = real.shape[0]
    if timeperiod > n:
        return
    ...
    for today in range(lookback, n):
        ...
        out[today] = b + m * float(timeperiod)


def TSF(real, timeperiod: int = 14):
    """
