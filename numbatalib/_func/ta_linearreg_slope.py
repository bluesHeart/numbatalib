...
@njit(cache=True)
def _linearreg_slope_kernel(real: np.ndarray, timeperiod: int, out: np.ndarray) -> None:
    n = real.shape[0]
    if timeperiod > n:
        return
...
def LINEARREG_SLOPE(real, timeperiod: int = 14):
    """
    ...
    _linearreg_slope_kernel(real_arr, tp, out)
    return out
