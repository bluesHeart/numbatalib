...
@njit(cache=True)
def _aroonosc_kernel(high: np.ndarray, low: np.ndarray, timeperiod: int, out: np.ndarray) -> None:
    n = high.shape[0]
    if n <= timeperiod:
        return
    ...
    for today in range(timeperiod, n):
        ...
        aroon_up = factor * (timeperiod - (today - highest_idx))
        aroon_down = factor * (timeperiod - (today - lowest_idx))
        out[today] = aroon_up - aroon_down
