...
@njit(cache=True)
def _ht_trendmode_kernel(real: np.ndarray, out: np.ndarray) -> None:
    n = real.shape[0]
    lookback_total = 63
    if n <= lookback_total:
        return
    ...
    while today <= end_idx:
        ...
        # Sine/leadSine crossing resets trend count.
        ...
        if float(days_in_trend) < (0.5 * smooth_period):
            trend = 0
        ...
        if (trendline != 0.0) and (math.fabs((temp_real - trendline) / trendline) >= 0.015):
            trend = 1
...
def HT_TRENDMODE(real):
    ...
    Returns an int array (0/1).
    """
    real_arr = as_1d_float64(real)
    out = nan_like(real_arr, dtype=np.int32)
    _ht_trendmode_kernel(real_arr, out)
    return out
